    ...
    for (unsigned row = 0; row < 65536; ++row) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (line[i] == 0) {
                ...
                //     line[i]++;
                // }
                // Prevent two 2048 tiles from merging into a 4096
                if (line[i] == 0xb) { // 2048 tile
...
// Norfe
static const float CPROB_THRESH_BASE = 0.00001f;
...
float score_toplevel_move(board_t board, int move) {
    ...
    // TODO:Norfe
    state.depth_limit = std::max(5, count_distinct_tiles(board)-1);
...
static board_t draw_tile(board_t board) {
    int max_rank = get_max_rank(board);  // 获取当前板上最大的数字

    int rand_val = unif_random(100);  // 生成一个 0 到 99 之间的随机数
    ...
    if (max_rank >= 0xb) {
        ...
        // 默认的生成规则，生成 2 或 4
        return (rand_val < 90) ? 1 : 2; // 90% 概率生成 2，10% 概率生成 4
    }
}
...
void play_game(get_move_func_t get_move) {
    ...
    while(1) {
        ...
        board_t tile = draw_tile(newboard);
